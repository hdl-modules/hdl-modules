...
--
-- If the packet length specified by the user equates more than one AXI burst, the core will
-- perform burst splitting internally.
--
--
...
--
-- 1. Maximum length bursts will be used.
--
-- 2. The ``AW`` transaction is only initiated once we have at least one ``W`` beat available.
--
-- 3. Backpressure from ``AWREADY`` and ``WREADY`` is of course respected.
--
-- 4. ``BREADY`` is always high.
--
...
begin
  ...
  axi_block : block
    ...
    constant split_bursts : boolean := packet_length_axi_beats > max_axi_burst_length_beats;

    function get_axi_bursts_per_packet return positive is
      constant max_axi_burst_length_bytes : positive := (
        max_axi_burst_length_beats * axi_data_width_bytes
      );
    begin
      if split_bursts then
    ...
    constant axi_bursts_per_packet : positive := get_axi_bursts_per_packet;

    constant axi_burst_length_beats : positive := packet_length_axi_beats / axi_bursts_per_packet;
  ...
  begin
    ...
    begin
      report "axi_bursts_per_packet: " & integer'image(axi_bursts_per_packet);
    ...
    begin
      ...
      simple_ring_buffer_manager_inst : entity ring_buffer.simple_ring_buffer_manager
        generic map (
          ...
          -- One address segment per AXI burst.
          -- Which is not necessarily the same as one packet.
          segment_length_bytes => axi_burst_length_bytes,
          -- Update 'buffer_written_address' once the whole packet has been written.
          segments_per_packet => axi_bursts_per_packet
        )
        port map (
          clk => clk,
          --
      ...
      assign_last_inst : entity common.assign_last
        generic map (
          packet_length_beats => axi_bursts_per_packet
        )
        port map (
          clk => clk,
          --
